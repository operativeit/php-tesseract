...
template<class T> typename \
  std::enable_if<hasTo##ptype<T, phpType<ptype>::type() const>::value, zend_result>::type \
castObjectTo##ptype(zend_object *obj, zval *dest) { \
  phpType<ptype>::make(dest, toObject<T>(obj)->to##ptype()); \
  return SUCCESS; \
} \
template<class T> typename \
  std::enable_if<!hasTo##ptype<T, phpType<ptype>::type() const>::value, zend_result>::type \
castObjectTo##ptype(zend_object *obj, zval *dest) { return FAILURE; }
...
int>::type compareObjectToSimilar(const T *a, const T &b) { \
  return a->compare(b); \
} \
\
template<class T> typename \
...
int>::type compareObjectToZval(const T *a, const zval *b) { \
  return a->compare(b); \
} \
\
template<class T> typename \
...
int>::type compareObjectTo##ptype(const T *a, \
...
cloneObject(zend_object *oldobj) { \
  const T *old = toObject<T>(oldobj); \
...
template<class T>
zend_result castObject(zend_object *obj, zval *dest, int type) {
  switch (type) {
    case IS_UNDEF:  ZVAL_UNDEF(dest); return SUCCESS;
    case IS_NULL:   ZVAL_NULL(dest);  return SUCCESS;
    case _IS_BOOL:  return castObjectToBool<T>(obj, dest);
    case IS_LONG:   return castObjectToLong<T>(obj, dest);
    case IS_DOUBLE: return castObjectToDouble<T>(obj, dest);
    case IS_STRING: return castObjectToString<T>(obj, dest);
    case IS_ARRAY:  return castObjectToArray<T>(obj, dest);
    case IS_OBJECT: {
      zval src;
      ZVAL_OBJ(&src, obj);
...
template<class T>
int compareObject(zval *a, zval *b) {
  ...
  if ((Z_TYPE_P(b) == IS_OBJECT) &&
    ...
    return compareObjectToSimilar<T>(obj, *toObject<T>(b));
  }

  switch (Z_TYPE_P(b)) {
    case IS_UNDEF:
    case IS_NULL:
      return compareObjectToNull<T>(obj);
#define P3_COMPARE_T(ptype) \
    case ptype: \
      return compareObjectTo##ptype<T>(obj, phpType<ptype>::get(b));
P3_COMPARABLE_TYPES(P3_COMPARE_T)
#undef P3_COMPARE_T
  }
  return compareObjectToZval<T>(obj, b);
}
